...
Sub Type_52(ByVal fullString As String)
    ...
    '"(P)" 是可能不存在
    '故得先拆解之
        Second_Part_needValue_list = ExtractParts(GetSecondPartOfString(fullString))
        PipeSize = Second_Part_needValue_list(0)
        Pad_symbol = Second_Part_needValue_list(1)
        
    '開始判定GetThirdPartOfString (fullString) 第三段的Insulation
        Third_Part_needValue_list = ExtractParts(GetThirdPartOfString(fullString))
        Insulation_Value = Third_Part_needValue_list(0)
        Matirial_Value = Third_Part_needValue_list(1)
    
    If Insulation_Value = "N/A" Then
        Insulation_Value = ""
    End If
                
                
    If Matirial_Value = "" Then
        Matirial_Value = "A36/SS400"
    End If
        
        
    
    
    '開始判定GetFourthPartOfString(fullString) 第四段的HOPS
        Hops_Value = GetFourthPartOfString(fullString)
        If Hops_Value = "N/A" Then
            Hops_Value = 150
        End If
    
    '開始判定GetFifthPartOfString(fullString) 第五段的LOPS
        Lops_Valie = GetFifthPartOfString(fullString)
        If Lops_Value = "N/A" Then
            Lops_Value = 500
        End If
        
    Set ws_For_52_Type_Table = Worksheets("For_52_Type_data")
...
' Plate_Size_b denotes the longer edge of the plate. The calculation for this is: Lops plus twice 25, added to twice E.
        Set PipeDetails = CalculatePipeDetails(PipeSize, Pipe_Thickness)
        If GetLookupValue(Replace(PipeSize, "B", "")) < 10 Then
            
        'Here for the Pipe Size < 10B
        PAD_Calculator_A = (((PipeDetails.Item("DiameterInch") / 2) + PipeDetail.Item("PipeThickness")) * 2) * (4 * Atn(1)) '((該置放管直徑/2)+該管徑的厚度 *2)*(4*atn(1))
        PAD_Calculator_B = Application.WorksheetFunction.VLookup(GetLookupValue(PipeSize), ws_For_52_Type_Table("A:F"), 6, False) * 2 + Lops_Value
        PAD_Calculator_t = PipeDetails.Item("PipeThickness")
        
        
        
        ElseIf GetLookupValue(Replace(PipeSize, "B", "")) >= 10 Then
        'Here for the Pipe Size >= 10B
        PAD_Calculator_A = (((PipeDetails.Item("DiameterInch") / 2) + PipeDetail.Item("PipeThickness")) * 2) * (4 * Atn(1))
        PAD_Calculator_B = Application.WorksheetFunction.VLookup(GetLookupValue(PipeSize), ws_For_52_Type_Table("A:F"), 6, False) * (2 + Lops_Value) + (25 * 2)
        PAD_Calculator_t = PipeDetails.Item("PipeThickness")
