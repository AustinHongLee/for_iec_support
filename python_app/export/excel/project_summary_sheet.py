...
def _write_project_summary_sheet(ws, project: ProjectAnalysisResult, summary: MaterialSummary, plans: list[CuttingPlan]):
    ...
    row += 1
    workbook_index = [
        ("專案摘要", "專案用了哪些 Type、總體重量、是否有錯誤或需確認項。", "A4 直式，第一頁優先列印"),
        ("重量明細表", "工程審查用明細；每列含 Type、單件數量、組數、總重與計算式。", "欄位多，建議橫式或用篩選"),
        ("計算標準與假設", "引用標準、資料狀態圖例、各 Type 計算可信度彙整。", "A4 直式"),
        ("支撐分類統計", "採購/施工分類彙總，例如 Pipe Shoe、U-Bolt、CS/SUS 支撐。", "A4 直式摘要"),
        ("支撐統計明細", "分類規則命中與需確認原因的逐筆追蹤。", "欄位多，建議橫式"),
    ...
    notes = [
        ...
        "若分類顯示需確認，請先看支撐統計明細，再決定是否更新分類規則。",
    ]
    for note in notes:
        row += 1
        ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=9)
