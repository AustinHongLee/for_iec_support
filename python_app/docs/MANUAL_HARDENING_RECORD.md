...
| Type 52/66 HOPS/LOPS priority | If the designation carries explicit `HOPS` / `LOPS`, those values override D-80 table/default dimensions. Example: `52-1/2B-A-150-200` uses `HOPS=150`, `LOPS=200`; only `52-1/2B-A` falls back to D-80/default `LOPS=150`. | `core/pipe_shoe_engine.py` | `validate_tables.py` |
