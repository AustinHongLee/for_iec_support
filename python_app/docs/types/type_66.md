...
> **分類**: 支撐型式（管線介面層） &nbsp;|&nbsp; **狀態**: ⚠️ 來源別、部分可加工
> **來源**: 中威 E25-24／中鼎 22A_5123A／中鼎 20E4588
> **圖面頁**: D-80、D-80A、D-80B；22A 另有 D-80C

**管鞍座（Pipe Shoe / Saddle）**——管線與支撐結構之間的曲面適配介面層。
包覆角 120°。適用範圍與分支上限依來源圖而異，不能再把單一 42 吋表當成三套專案的共同真值。
是 Type 46 / Type 47 圖面標註 "DETAIL SEE D-80 (TYP.)" 的實體定義。

## 目前加工圖安全界線（2026-07-29）

| 來源 profile | 已核定可下料範圍 | 明確停算範圍 |
|---|---:|---|
| `cw_e25_24_hp6` | D-80 ≤8" | 10–24" 補強件輪廓未齊；D-80B 26–50" 尚缺 NO.1–5 完整展開／定位 |
| `ctci_22a_5123a` | D-80 ≤5" | 6–24" 補強件輪廓未齊；D-80B 26–42"、D-80C 44–78" 尚缺逐件 CAD recipe |
| `ctci_20e4588` | D-80 ≤8" | 10–24" 補強件輪廓未齊；D-80B 僅有 26/28" 表值且仍缺逐件展開 |

已核定結果保存構件 ID、原圖／版次、H 型鋼完整規格、切長、120° 鞍弧、
補強墊板展開尺寸、Ø6 洩水孔與焊接規則。其餘範圍雖保留原圖尺寸表，
但不再用 `HOPS × A` 等估重矩形冒充加工尺寸。

> 本頁以下舊表主要描述中威／既有基準，僅作 UI 人工解讀輔助；來源別計算真值以
> `configs/type_66_fabrication.json`、`pipe_shoe_engine.py`、原始 PDF 與 golden tests 為準。
